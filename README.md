...
| 球拍 | 形状 | 弹性 | 特性 |
|---|---|---|---|
| 滑板 | 滑板造型（翘头板面 + 轮子） | 0.70 | 反弹直接，移动最快（900 px/s） |
| 橄榄球 | 凸面椭圆 | 0.95 | 顶面弧形、法线连续变化——中央垂直、两侧渐偏，移动中速（775 px/s） |
| 碗 | 凹面弧壁 | 0.50 | 收住小球并向上定向送出，可控性最强，移动较慢（625 px/s） |

- 移动：**键盘主控** ← → 或 A / D（平滑加速，手感顺滑）
- 发球：空格（球停在球拍上时；点击亦可）
- 选择球拍：菜单中按 1 / 2 / 3
- 每次球拍成功接球反弹：出射速度 = 接近速度 × **1.30**（只增不减，上限 **1400 px/s**；
  经物理仿真标定，颠球 3-4 次后球速足以够到砖块区并击破普通砖，见 `docs/design-v1.md`）
- 每次反弹附带 ±1.5% 随机横向微扰动，避免高速"垂直无限弹"轨道
- 碗为半透明材质，球进入碗内仍可见

**砖块（默认 8×6 阵，窄窗口自动缩列；自上而下由硬到软）**

| 类型 | 行 | 生命 | 破砖冲击阈值 |
|---|---|---|---|
| 高阻力砖 | 顶 2 行 | 1 | 500 px/s |
| 普通砖 | 中 2 行 | 1 | 450 px/s |
| 软砖 | 底 2 行 | 1 | 300 px/s |
...
- 物理单位：像素（100px = 1m），`length_unit = 100` 避免 Rapier 默认 400 m/s 的速度钳制
  变成 400px/s 硬上限（见 git log 0.3.5）；重力 900 px/s²（0.5.6 重标定）。
